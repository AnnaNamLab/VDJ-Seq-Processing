...
cell_type_annotation=read.csv('/Users/saramoein/downloads/2024-08-13_CellMetadata_HL1-24incHL8R_RetainedCellsOnly_MainCellTypeAndSubtypeNames.csv')
...
##### keeping only singlet and lo_confidern_singlet
...
HRS_FILTERED_out_clone_dominantChain= HRS_FILTERED_out_clone_dominantChain[HRS_FILTERED_out_clone_dominantChain$cell_id1 %in% union(singlet$V1, singlet_low_confidential$V1),]
...
d3=NA
j1="IGKJ1*01"
j2="IGKJ4*02"#IGLJ2*01"
j3="IGKJ4*01"#IGKJ3*01"

# for (i in 1:nrow(merge_igblast_trust4)){
#   
#   #searching any igblast rows that has dominat tVDJ genese to extract top 3 V/D/J genes
#   if ((chain=="IGL" | chain=="IGK") & grepl(dominant_V,merge_igblast_trust4$igblast_v_call[i],fixed =TRUE) & grepl(dominant_J,merge_igblast_trust4$igblast_j_call[i] ,fixed =TRUE)){
#     
#     v1=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 1)
#     v2=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 2)
#     v3=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 3)
#     
#     
#     j1=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 1)
#     j2=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 2)
#     j3=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 3)
#    
#     if (is.na(v1)==FALSE & is.na(v2)==FALSE & is.na(v3)==FALSE & is.na(j1)==FALSE & is.na(j2)==FALSE & is.na(j3)==FALSE){
#       break
#     }
#     
#   }
#   
#   if ((chain=="IGH") & grepl(dominant_V,merge_igblast_trust4$igblast_v_call[i],fixed =TRUE) & grepl(dominant_D , merge_igblast_trust4$igblast_d_call[i],fixed =TRUE) & grepl(dominant_J,merge_igblast_trust4$igblast_j_call[i] ,fixed =TRUE)){
#     
#     v1=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 1)
#     v2=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 2)
#     v3=sapply(strsplit(merge_igblast_trust4$igblast_v_call[i], ',\\s*'), `[`, 3)
#     
#     d1=sapply(strsplit(merge_igblast_trust4$igblast_d_call[i], ',\\s*'), `[`, 1)
#     d2=sapply(strsplit(merge_igblast_trust4$igblast_d_call[i], ',\\s*'), `[`, 2)
#     d3=sapply(strsplit(merge_igblast_trust4$igblast_d_call[i], ',\\s*'), `[`, 3)
#     
#     j1=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 1)
#     j2=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 2)
#     j3=sapply(strsplit(merge_igblast_trust4$igblast_j_call[i], ',\\s*'), `[`, 3)
#     
#     if (is.na(v1)==FALSE & is.na(v2)==FALSE & is.na(v3)==FALSE & is.na(j1)==FALSE & is.na(j2)==FALSE & is.na(j3)==FALSE & is.na(d1)==FALSE & is.na(d2)==FALSE & is.na(d3)==FALSE){
#       break
#     }
#     
#   }
