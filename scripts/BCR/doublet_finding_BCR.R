...
# ------------------------------------------------------------
# Function to detect overlap of cell IDs between two datasets
# separately for HRS and Bcells
# ------------------------------------------------------------
...
# Used to detect peaked vs flat read distributions
# ------------------------------------------------------------
kurtosis <- function (x) {
  n <- length(x)
...
# Measures diversity of reads across contigs
# ------------------------------------------------------------
...
# ------------------------------------------------------------
thre=0.7 # dominance threshold for singlet
entropy_ther=0.8 # entropy threshold
...
# detecting the cell status for all sampels  
 ...
 dir.create(paste0('./',sub_folder))
 dir.create(paste0('./',sub_folder,'/percentage'))
...
# ------------------------------------------------------------
# Per-cell classification using kurtosis and entropy
# ------------------------------------------------------------
...
for (i in 1:length(cells$id)){ 
  ...
  # Compute read proportions, kurtosis, and entropy
  ...
  if ((max(agg_tbl$read_fragment_count_group) == read_thre) &(nrow(agg_tbl)==1)){
  ...
  } else if (nrow(dominant_singlet)>0){
    singlet=rbind(singlet,c(cells$id[i],ka,ent))
...
# ------------------------------------------------------------
# Clean up NA rows from preliminary classifications
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Restrict valid singlets to biologically relevant cell types
# Only Bcells and HRS can be true singlets
# Others are reclassified as doublets / not assigned
# ------------------------------------------------------------
...
# Singlets outside Bcell/HRS are considered other doublets
...
# Keep only valid singlets from Bcells and HRS  
...
# ------------------------------------------------------------
# Refinement step:
# Handle Inf / -Inf / NaN kurtosis or insufficient reads
# Threshold for insufficient reads = 7
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Assign ReadStatus and biological cellType
# ------------------------------------------------------------
...
# Add kurtosis and entropy values
