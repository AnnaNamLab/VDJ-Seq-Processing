...
library(ggplot2)
#
for (sample in c("HL1","HL8","HL8R", "HL10","HL15","HL16","HL20","HL21")){
              ...
              trust4_out_barcode_airr= read.table(Sys.glob('out_*.tsv'),header= FALSE, sep='\t')
              ...
              pilot_cellStatus_file= read.csv(Sys.glob(paste0('/Users/saramoein/Documents/new_run_HL_May2025/FINAL_doublets_TCR_thre07_ent08/',sample,'_Rawdata_TRA.csv')))
              singlet= unique(pilot_cellStatus_file$cell_id1[pilot_cellStatus_file$final_ReadStatus=="singlet"])
              
              
              TRA_new= TRA_new[TRA_new$cell_id1 %in% singlet,]
              TRA_new_Tcell= TRA_new_Tcell[TRA_new_Tcell$cell_id1 %in% singlet,]
              TRA_new= TRA_new %>% group_by(cell_id1) %>% dplyr::slice(which.max(read_fragment_count)) 
              
              TRA_new <- TRA_new %>%
                group_by(malignantStatus) %>%
                group_modify(~ if (.y$malignantStatus == "Tcells") {
                  .x %>%
                    group_by(cell_id1) %>%
                    slice_max(order_by = read_fragment_count, n = 1, with_ties = FALSE) %>%
                    ungroup()
                } else {
                  .x
                }) %>%
              ...
              chain="TRB"
              pilot_cellStatus_file= read.csv(Sys.glob(paste0('/Users/saramoein/Documents/new_run_HL_May2025/FINAL_doublets_TCR_thre07_ent08/',sample,'_Rawdata_TRB.csv')))
              singlet= unique(pilot_cellStatus_file$cell_id1[pilot_cellStatus_file$final_ReadStatus=="singlet"])
              
              TRB_new= TRB_new[TRB_new$cell_id1 %in% singlet,]
              TRB_new_Tcell= TRB_new_Tcell[TRB_new_Tcell$cell_id1 %in% singlet,]
              ...
              for (HRS_celltype in c("HRS-CD4T","HRS-CD8T")){
                ...
                for (i in c(1:5)){
                  ...
                  ##########################################################################################
                  # number of cells from intersect the total  #  number of cells from intersect the total #
                  # cells in clones with more                  #  cells in clones with less                #
                  # than ther size and in HRS with Tcells       #  than ther size and in HRS with Tcell      #  
                  ##########################################################################################
