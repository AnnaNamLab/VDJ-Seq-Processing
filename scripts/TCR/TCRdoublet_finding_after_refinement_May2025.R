...
for (sample in c("HL1","HL10","HL21","HL8","HL8R","HL15","HL20","HL16","HL12")){
  ...
  #colnames(data_raw) =c(consensus_id	index_within_consensus	V_gene	D_gene	J_gene	C_gene	CDR1	CDR2	CDR3	CDR3_score	read_fragment_count CDR3_germline_similarity complete_vdj_assembly)
  ...
  for (j in 1:2){ ### THROUGH 3 CHAINS IGL_IGK/TRB
    ...
    for (i in 1:length(cells$id)){ ## per cell we decide what is it. Each cell can be either singlet, or doublet or noise
      ...
      ent= normalized_entropy(agg_tbl$V11_group)
      
      
      if ((max(agg_tbl$V11_group) == read_thre) &(nrow(agg_tbl)==1)){#### if the cell's maximum read count is less than the read_thre (here is 2) then that cell is noise
      ...
      } else if (is.na(ka) | (is.infinite(ka) & sum(agg_tbl$V11_group)<=3)){ 
        #  
    ...
    # Step 1: Aggregate V11 by main_vdjcall within each cell_id1
    vdj_aggregated <- rawCDR3_ALLcells %>%
      group_by(cell_id1, main_vdjcall) %>%
      summarise(aggregated_read_count = sum(V11, na.rm = TRUE)) %>%
    ...
    cell_info <- rawCDR3_ALLcells %>%
      group_by(cell_id1) %>%
      summarise(
        total_reads = sum(V11, na.rm = TRUE),  # Total reads per cell_id1
    ...
    # } else {
      ...
      data_sub_if_Not_Assigned_with_lv <- data_sub_if_Not_Assigned %>%
        ...
        slice_max(order_by = V11, n = 2, with_ties = FALSE) %>%
    ...
    rawCDR3_ALLcells$CDR1_length = nchar(rawCDR3_ALLcells$V7)
    rawCDR3_ALLcells$CDR2_length = nchar(rawCDR3_ALLcells$V8)
    ...
    if (j==2){
      ...
      ind_high= which((rawCDR3_ALLcells$status_impute =="doublet") & (rawCDR3_ALLcells$V13==1))
      ind_med= which((rawCDR3_ALLcells$status_impute =="doublet") &  (rawCDR3_ALLcells$CDR1_length <= 23  | rawCDR3_ALLcells$CDR2_length <= 20 | (rawCDR3_ALLcells$V13==0)) & (rawCDR3_ALLcells$CDR1_length !=1  | rawCDR3_ALLcells$CDR2_length !=1))
      ...
      ind_high= which(rawCDR3_ALLcells$status_impute =="doublet"  & (rawCDR3_ALLcells$V13==1))
      ind_med= which(rawCDR3_ALLcells$status_impute =="doublet" &  (rawCDR3_ALLcells$CDR1_length <= 17  | rawCDR3_ALLcells$CDR2_length <= 8 | (rawCDR3_ALLcells$V13==0)) & (rawCDR3_ALLcells$CDR1_length !=1  | rawCDR3_ALLcells$CDR2_length !=1))
      ind_low = which(rawCDR3_ALLcells$status_impute =="doublet" & (rawCDR3_ALLcells$CDR1_length ==1  & rawCDR3_ALLcells$CDR2_length ==1))
      rawCDR3_ALLcells$doublet_confidence[ind_high]="doublet_high"
      rawCDR3_ALLcells$doublet_confidence[ind_med]="doublet_medium"
      rawCDR3_ALLcells$doublet_confidence[ind_low]="doublet_low"
